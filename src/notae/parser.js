...
export function parseSolemnes(gabc, { oct = 3, initialClef, weights } = {}) {
  let clavis = initialClef || "c3";
  const words = (gabc || "")
    .replace(/\)\s(?=[^\)]*(?:\(|$))/g, ")\n")
    .split(/\n/g)
    .map((line) => parse_word(line, oct, () => clavis, (v) => { clavis = v; }, resolveRubrics(weights)))
...
function _legacyResolveRubrics(weights) {
  return { ictusFactor:1, ictusDuration:0.25, moraFactor:0.5, moraDuration:1,
           initioDebilisAttenuation:2, punctumInclinatumAttenuation:1, liquescentAttenuation:0.5,
           pressusDuration:0.5, pressusOffset:1, oriscusAttenuation:1 };
}

function parse_word(gabcWord, oct, getClef, setClef, weights) {
  const matches = []; let m;
  ...
  return matches.map((mm) => {
    ...
    const part = parse_notation({ lyric, notation, oct, getClef, setClef, weights });
    return Array.isArray(part) ? part : [part];
  }).flat();
}

function parse_notation({ lyric, notation, oct, getClef, setClef, weights }) {
  if (!notation) return;
  const l = (lyric[0] || "");
  ...
  if (vowel) {
    ...
    vowel = vowel.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();
    if (vowel === "y") vowel = "i";
  }
  ...
  const flushNeume = () => {
    ...
    const notes = parse_neume(buf, oct, tictus, getClef(), weights);
    out.push({ type: "neume", vowel, notes, lyric: l, notation: buf.join("") });
    buf = [];
  };

  for (const tok of notation) {
    if (CLEFS.has(tok)) { // clef change anywhere
      flushNeume();
      setClef(tok);
      continue;
    }
    if (DIVISIONES.has(tok)) { // end current neume, then emit divisio
      flushNeume();
      out.push({ type: "diviso", divisio: tok, notes: [{ step: null, arsis: 0, ictus: 0, duration: DIVISIONES.get(tok), vowel: "" }] });
      continue;
    }
    if (tok === " " || tok === "z" || tok === "Z" || tok === "z0" || tok === "{" || tok === "\r") continue;
...
function parse_neume(neume, oct, tict, clavis, weights) {
  ...
  for (let i=0; i<neume.length; i++) {
    let raw = neume[i];
    if (["!", "/", "//"].includes(raw)) { breaks.push(i); continue; }
    ...
    let arsis = 5, ictus = 0, duration = 1;
    const prev = notes.length ? notes[notes.length-1] : undefined;

    // initio debilis
    if (raw[0] === "-") { raw = raw.slice(1); arsis -= weights.initioDebilisAttenuation; }

    // pitch
    const clefOffset = (CLEFS.get(clavis) || 0);
    const pos = raw[0].toLowerCase().charCodeAt(0) - "a".charCodeAt(0) - 6 - clefOffset;
    ...
    // Rule 1: episemata
    if (raw.includes("'")) { arsis += weights.ictusFactor; ictus = 1; duration += weights.ictusDuration; }
    if (raw.includes("_")) { arsis += weights.ictusFactor; ictus = 1; duration += weights.ictusDuration; }

    // Rule 2: sustained notes
    if (raw.includes("..") || raw.includes(".")) {
      arsis += weights.moraFactor; duration += weights.moraDuration; ictus = 1;
    }
    if (raw.includes("ss") || raw.includes("vv")) {
      arsis += weights.moraFactor; ictus = 1; duration += weights.moraDuration;
      if (raw.includes("sss") || raw.includes("vvv")) duration += weights.moraDuration;
    }

    // quilisma prep
    if (raw.toLowerCase().includes("w")) { if (prev) prev.arsis += 1; arsis = Math.max(0, arsis - 0.5); }

    // Rule 3: group begins (and tictus)
    if (i === 0 || (tict && i === 2)) { arsis += 1; if (neume.length > 1) { ictus = 1; arsis += 0.5; } }

    // liquescent / punctum inclinatum
    if (raw.includes("~") || raw.includes("<") || raw.includes(">")) { arsis -= 0.5; duration -= 0.5; }
    if (raw[0] === raw[0].toUpperCase()) { arsis -= 1; duration -= 0.25; }
  ...
  breaks.forEach((b, bi) => { const idx = b - bi; const n = notes[idx]; if (n) { n.arsis += 1; n.ictus = 1; } });
  return notes;
}

export const solemnes = (gabc, oct = 3) => parseSolemnes(gabc, { oct }).words;
