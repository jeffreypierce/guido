import { resolveProsody } from './presets.js';

export function prosody(events, intonation, { prosody: preset, modeTenorPc } = {}) {
  const P = resolveProsody(preset);
  const out = new Array(events.length);
  const isPhraseStart = (i) => i === 0 || (events[i-1] && events[i-1].type === 'rest');
  ...
  const normPhrase = (start, end) => {
    ...
    const v = [];
    for (let k = start; k < end; k++) {
      if (events[k].type !== 'note') { v[k - start] = 0; continue; }
  ...
  while (i < events.length) {
    ...
    for (let k = i; k < j; k++) {
      const ev = events[k];
      if (ev.type === 'rest') { out[k] = { velocity: 0, predelay: 0, durationScale: 1 }; continue; }
      const idx = k - i;
      let v = norm[idx];
      ...
      if (ino && typeof modeTenorPc === 'number' && typeof ino.pc === 'number') {
        const dist = Math.min((12 + ino.pc - modeTenorPc) % 12, (12 + modeTenorPc - ino.pc) % 12);
      ...
      if (next && next.type === 'rest' && typeof next.divisio === 'string') {
        const strength = next.divisio.includes('::') ? 1.0 :
                         (next.divisio.includes(':') || next.divisio.includes(';')) ? 0.7 :
                         (next.divisio.includes(',')) ? 0.4 : 0.0;
