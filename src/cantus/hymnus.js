// src/cantus/hymnus.js — hymn selection (OF/EF), initial heuristic implementation

import { cantus } from './cantus.js';
import { norm, tokens } from '../aux/aux.js';

function categoryTags(festum) {
  const la = norm(festum.title_la || festum.title || '');
  const tags = new Set();
  const add = (b, t) => { if (b) tags.add(t); };
...
function seasonHints(season) {
  switch (season) {
    case 'ad': return [/veni/, /adven/];
    case 'lt': return [/parce|audi|attende|jejun|lenten|quadragesim/];
    case 'ea': return [/resur|pasch|allelu/];
    case 'ct': return [/nativ|epiph/];
    default: return [];
  }
}
...
function scoreHymn(festum, row) {
  ...
  const catMap = {
    apostle: [/apostol/], evangelist: [/evangel/], martyr: [/martyr/], virgin: [/virgin|virgo/],
    bishop: [/pontific|episcop/], confessor: [/confessor/], doctor: [/doctor/], bvm: [/mari/],
  };
  for (const c of cats) for (const rx of (catMap[c] || [])) if (rx.test(inc)) s += 2;
...
export function hymnus(ctx, opts = {}) {
  ...
  const sources = Array.isArray(opts.source) ? opts.source : opts.source ? [opts.source] : ['LH'];
  const offices = ['hy'];
  const modes = opts.modes || [];
  const cand = [];
  for (const src of sources) cand.push(...cantus({ offices, modes, source: src }));
  if (opts.includeAM && !sources.includes('AM')) cand.push(...cantus({ offices, modes, source: 'AM' }));
  const scored = cand.map((r) => ({ row: r, score: scoreHymn(festum, r) }))
