...
function parsePageHints(str) {
  ...
  while ((m = rangeRe.exec(s))) {
    ...
    if (Number.isFinite(a) && Number.isFinite(b))
      pages.push({ from: Math.min(a, b), to: Math.max(a, b) });
  }
  ...
  while ((m = singleRe.exec(s))) {
    ...
    const covered = pages.some((r) =>
      typeof r === "object" && r && "from" in r
        ? n >= r.from && n <= r.to
        : n === r
    );
...
async function main() {
  ...
  const EMIT_JSON =
    process.argv.includes("--emit-json") || process.env.EMIT_JSON === "1";
  ...
  try {
    ...
    index._meta.sources.push({
      id: "LU_calendar",
      path: path.relative(repoRoot, luCalPath),
      schema: luCal?.schema || null,
    });
  ...
  try {
    ...
    index._meta.sources.push({
      id: "LH_feast_pages",
      path: path.relative(repoRoot, lhFeastPagesPath),
    });
  } catch {}
  try {
    ...
    index._meta.sources.push({
      id: "LH_english_adaptation",
      path: path.relative(repoRoot, lhEnglishAdaptPath),
    });
  } catch {}
  ...
  try {
    ...
    index._meta.sources.push({
      ...
      path: path.relative(repoRoot, luSaintsPath),
    });
  ...
  try {
    ...
    index._meta.sources.push({
      ...
      path: path.relative(repoRoot, potPath),
    });
    ...
    for (const [secName, secObj] of Object.entries(sections)) {
      if (secObj && typeof secObj === "object") {
        for (const [key, node] of Object.entries(secObj)) {
          ...
          {
            const m = key.match(
              /^(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)_(\d+)_to_(\d+)$/i
            );
            if (m) {
              const mon = MONTHS_MAP.get(m[1].toLowerCase());
              const a = Number(m[2]),
              ...
              if (mon && Number.isInteger(a) && Number.isInteger(b) && a <= b) {
                ...
                for (let d = a; d <= b; d++) {
                  if (pages.length) {
                    addPropersPages(`${pad2(mon)}-${pad2(d)}`, pages);
                    addedPOT++;
                  }
                }
              }
          ...
          {
            const m = key.match(
              /^(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)_(\d+)$/i
            );
            if (m) {
              const mon = MONTHS_MAP.get(m[1].toLowerCase());
              const d = Number(m[2]);
          ...
          if (pages2.length) {
            ...
            if (LM.some((t) => keyNorm.includes(t)))
              POT_MAP[key] = Array.from(new Set(pages2));
            // Seasonal Sundays like first/second/third/fourth
            const ORD = { first: 1, second: 2, third: 3, fourth: 4, fifth: 5 };
            const mSun = keyNorm.match(
              /^(first|second|third|fourth|fifth)_sunday$/
            );
            if (mSun) {
              const n = ORD[mSun[1]];
              const sec =
                secName && String(secName).toLowerCase().replace(/\s+/g, "_");
              if (n && sec)
                POT_MAP[`${sec}_sunday_${n}`] = Array.from(new Set(pages2));
            }
    ...
    if (addedPOT)
      console.log(`${check} LU_proper_of_time → EF day pages: ${addedPOT}`);
  ...
  function ensureShape(rec) {
    ...
    if (!("dow" in rec)) rec.dow = null; // 'dominica'|'feria' when known
    return rec;
  }
  ...
  if (index.EF && typeof index.EF === "object") {
    for (const k of Object.keys(index.EF)) {
      const v = index.EF[k];
      if (Array.isArray(v))
        index.EF[k] = v.map((rec) => {
          const out = ensureShape(rec);
          out.rank = classToRankCode(
            out.class || out.class_rank,
            out.type_hint
          );
          if (out.season == null) out.season = seasonHintFromMMDD(k);
          // Never emit dow in dayIndex; resolve at runtime when needed
          delete out.dow;
  ...
  try {
    ...
    for (const k of Object.keys(index.EF)) {
      ...
      for (const rec of arr) {
        ...
        if (
          (rec._from === "lu_calendar" &&
            (!rec.title || rec.title === "Feria")) ||
          !rec.title ||
          rec.title === "Feria"
        ) {
  ...
  try {
    ...
    const pagesFromHints = (rec) => {
      ...
      return Array.from([...s].filter((n) => Number.isFinite(n) && n > 0)).sort(
        (a, b) => a - b
      );
    };
    let recsUpdated = 0,
    ...
    for (const k of Object.keys(index.EF)) {
      ...
      for (const rec of arr) {
        ...
        const priorCount =
          rec.propers && typeof rec.propers === "object"
            ? Object.values(rec.propers).reduce(
                (a, v) => a + (Array.isArray(v) ? v.length : 0),
                0
              )
            : Array.isArray(rec.propers)
            ? rec.propers.length
            : 0;
        const byOffice = {};
        ...
        if (Object.keys(sortedByOffice).length) {
          ...
          const afterCount = Object.values(sortedByOffice).reduce(
            (a, v) => a + v.length,
            0
          );
    ...
    console.log(
      `${check} propers: +${idsAdded} LU id(s) across ${recsUpdated} EF record(s)`
    );
  } catch (e) {
  ...
  try {
    ...
    let lhFeastPages = [];
    try {
      lhFeastPages = JSON.parse(
        await fs.readFile(path.join(etcDir, "LH_feast_pages.json"), "utf8")
      );
    } catch {}
    ...
    const stem = (t) =>
      t.replace(/(orum|arum|ibus|ium|ius|ae|is|us|um|am|as|os|e|i|es)$/, "");
    const tokens = (s) =>
      latinNorm(s)
        .split(/[\s&]+/)
        .filter(Boolean)
    ...
    const pageLabelToPages = Array.isArray(lhFeastPages)
      ? lhFeastPages.map((r) => ({
          label: r?.label || "",
      ...
      for (const rec of arr) {
        ...
        if (pages.length) {
          function idsForPages(pgList, alsoCollectPages = false) {
            ...
            return {
              ids: Array.from(new Set(acc)),
              usedPages: Array.from(used),
            };
          }
          ...
          if (usedPages && usedPages.length) {
            rec.hymns_pages = Array.from(
              new Set([...(rec.hymns_pages || []), ...usedPages])
            );
          } else {
            // keep original hint pages if nothing produced IDs
            rec.hymns_pages = Array.from(
              new Set([...(rec.hymns_pages || []), ...pages])
            );
          }
          if (idsFromPages.length) {
            const prev = Array.isArray(rec.hymns.lh) ? rec.hymns.lh : [];
            let merged = Array.from(new Set([...prev, ...idsFromPages]));
            if (merged.length > MAX_HYMN_IDS)
              merged = merged.slice(0, MAX_HYMN_IDS);
    ...
    console.log(
      `${check} hymns: +${hymnIdsAdded} candidates across ${recsHymnUpdated} EF record(s)`
    );
  } catch (e) {
