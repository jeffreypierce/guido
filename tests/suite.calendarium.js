...
it("movable feasts (Trinity, Corpus, Christ the King) map to exact dates", () => {
  for (const form of ["EF", "OF"]) {
    const L = form === "OF" ? lookupOF(y) : lookupEF(y);
...
it("fixed feasts from calendar.json appear on their month/day", () => {
  ...
  const aFixed = cal.find(
    (e) => Number.isInteger(e.month) && Number.isInteger(e.day)
  );
  if (!aFixed) {
    assert(true, "no fixed feasts present to test");
    return;
  }
...
it("movable overlay takes precedence over fixed feasts on same day", () => {
  ...
  for (const e of cal) {
    ...
    for (const m of cal.filter(
      (x) => !Number.isInteger(x.month) && !Number.isInteger(x.day)
    )) {
      const L62 = lookupEF(y);
      const L74 = lookupOF(y);
      ...
      if (mm === mm2 && dd === dd2) {
        ...
        if (row) {
          ...
          assert(
            row.id === m.id,
            `Movable ${m.id} should override fixed ${e.id} on ${ymd}`
          );
          return; // success
        }
      }
