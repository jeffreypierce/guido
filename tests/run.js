...
const C = (code) =>
  TTY ? (s) => `\x1b[${code}m${s}\x1b[0m` : (s) => String(s);
const bold = C(1),
  dim = C(2),
  red = C(31),
  green = C(32),
...
/** Deep equal for plain data */
globalThis.eq = (a, b) => JSON.stringify(a) === JSON.stringify(b);
globalThis.expectEq = (a, b, msg = "not equal") => assert(eq(a, b), msg);

/** Same UTC calendar day */
globalThis.sameDay = (a, b) => {
  const A = new Date(a),
    B = new Date(b);
  return (
...
globalThis.expectSameDay = (a, b, msg = "dates differ") =>
  assert(sameDay(a, b), msg);
...
if (suites.length === 0) {
  console.error(
    red("No test suites found in tests/. Expected files like suite.core.js")
  );
  process.exit(1);
}
