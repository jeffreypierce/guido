...
function sameDay(a,b){ const A=new Date(a),B=new Date(b);
  return A.getUTCFullYear()===B.getUTCFullYear() && A.getUTCMonth()===B.getUTCMonth() && A.getUTCDate()===B.getUTCDate();
}

it("Easter core relations hold for multiple years (1962)", () => {
  for (const y of [2024, 2025, 2030, 2038]) {
    const L = lookup1962(y);
    // Pentecost = Easter + 49
    assert(sameDay(L.pentecost, addDays(L.easter_sunday, 49)), "Pentecost relation failed " + y);
    // Trinity = Pentecost + 7
    assert(sameDay(L.holy_trinity, addDays(L.pentecost, 7)), "Trinity relation failed " + y);
    // Ash Wednesday = Easter - 46
    assert(sameDay(L.ash_wednesday, addDays(L.easter_sunday, -46)), "Ash Wed relation failed " + y);
  }
});

it("Transfers (1974): Ascension/Corpus/Epiphany toggle works", () => {
  const y = 2025;
  const L0 = lookup1974(y, { transfer: { epiphany:false, ascension:false, corpusChristi:false }});
  const L1 = lookup1974(y, { transfer: { epiphany:true,  ascension:true,  corpusChristi:true  }});
  ...
  assert(new Date(L0.corpus_christi).getUTCDay() === 4, "Corpus not Thu without transfer");
  assert(new Date(L1.corpus_christi).getUTCDay() === 0, "Corpus not Sun with transfer");
  // Epiphany: Jan 6 vs Sunday between Jan 2–8
  assert(new Date(L0.epiphany).getUTCMonth() === 0 && new Date(L0.epiphany).getUTCDate() === 6, "Epiphany not Jan 6 without transfer");
  assert(new Date(L1.epiphany).getUTCDay() === 0, "Epiphany not Sunday with transfer");
});
...
it("Mother of the Church and Immaculate Heart derived from Pentecost/ Sacred Heart", () => {
  ...
  const p1 = new Date(L.pentecost); p1.setUTCDate(p1.getUTCDate() + 1);
  ...
  const sh1 = new Date(L.sacred_heart); sh1.setUTCDate(sh1.getUTCDate() + 1);
