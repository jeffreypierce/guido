...
import { season1962, season1974, seasonNormalize } from "../src/festum/tempus.js";

banner("tempus — season classifiers");

function addDays(d, n){ const x = new Date(d); x.setUTCDate(x.getUTCDate()+n); return x; }

it("1962: returns one of the EF codes at key boundaries", () => {
  for (const y of [2024, 2025]) {
    ...
    assert(codes.has(season1962(addDays(L.pentecost,1), L)), "EF after Pentecost");
  }
});
...
it("seasonNormalize glues EF/OF codes for mass-selection", () => {
  ...
  assert(seasonNormalize("sg")==="lt" || seasonNormalize("sg")==="ot", "sg maps to a generic bucket");
});
...
it("splitOrdinary yields ot1 before Lent and ot2 after Pentecost", () => {
  for (const y of [2024, 2025]) {
    ...
    const midOT1 = new Date(L.baptism); midOT1.setUTCDate(midOT1.getUTCDate() + 7);
    const a = season1974(midOT1, L, { splitOrdinary: true });
    assert(a === 'ot1', `expected ot1 before Lent (${y})`);
    // A day well after Pentecost
    const midOT2 = new Date(L.pentecost); midOT2.setUTCDate(midOT2.getUTCDate() + 14);
