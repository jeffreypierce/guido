...
it("uses dayIndex propers when available", () => {
  ...
  for (const k of Object.keys(EF)) {
    ...
    if (arr.some((r) => r.propers && Object.keys(r.propers).length)) {
      mmdd = k;
      break;
    }
  }
  ...
  assert(
    inItem && Array.isArray(inItem.candidates) && inItem.candidates.length > 0,
    "no Introit candidates from index"
  );
});
...
it("potIndex fallback produces candidates for a landmark (Epiphany)", () => {
  ...
  const inItem = items.find((x) => x.office === "in");
  assert(
    inItem && inItem.candidates && inItem.candidates.length > 0,
    "no Introit candidates from potIndex"
  );
});
