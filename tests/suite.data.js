...
const ALLOWED_SEASONS = new Set(["ad","ct","lt","ea","ot","ot1","ot2","ap","sg","ot2"]);
...
it("masses entries have normalized codes", () => {
  ...
  if (massesData.masses) {
    for (const [k,v] of Object.entries(massesData.masses)) all.push({k, ...v});
  }
  if (Array.isArray(massesData.adlib)) {
    for (const v of massesData.adlib) all.push({k: v.title || "(adlib)", ...v});
  }
  for (const m of all) {
    for (const s of (m.seasons || [])) assert(ALLOWED_SEASONS.has(s), `bad season ${s} in ${m.title||m.k}`);
    for (const r of (m.ranks || []))   assert(ALLOWED_RANKS.has(r),   `bad rank ${r} in ${m.title||m.k}`);
    for (const d of (m.days || []))    assert(ALLOWED_DAYS.has(d),    `bad day ${d} in ${m.title||m.k}`);
  }
});
...
it("calendar movable IDs are resolvable by lookups", () => {
  const year = 2025;
  const movableIds = Array.from(ids(calendar)).filter(id => id && !/^\s*$/.test(id)).filter(id => {
    const e = calendar.find(x => x.id === id);
    return !(Number.isInteger(e.month) && Number.isInteger(e.day)); // not fixed
  });
  const L62 = lookup1962(year);
  const L74 = lookup1974(year);
